...
Then update `config/database.yml` to define the `queue` connection (template provided in `config/database.yml.SAMPLE`).
...
# production: the installer writes a systemd unit — see cg_write_solid_queue_service() in deploy/lib/common.sh
```
...
- [x] Solid Queue production service: generated by the installer (`cg_write_solid_queue_service()` in `deploy/lib/common.sh`) — no static example file needed.
